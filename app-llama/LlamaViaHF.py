...
class LlamaInferenceEngine:
    ...
    def __init__(
        ...
        self.metrics = {"model_load_time": 0.0, "total_inference_time": 0.0, "total_tokens_generated": 0, "average_tokens_per_second": 0.0}
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            pipeline_kwargs = {"model": self.model, "tokenizer": self.tokenizer, "torch_dtype": torch_dtype}
    ...
    def generate_text(
        ...
        try:
            ...
            if self.metrics["total_inference_time"] > 0:
                self.metrics["average_tokens_per_second"] = self.metrics["total_tokens_generated"] / self.metrics["total_inference_time"]
    ...
    def run_benchmark(self, prompts: List[str], num_generations: int = 1, **generation_kwargs) -> List[Dict[str, Any]]:
        """
        ...
        for i, prompt in enumerate(prompts):
            for gen_num in range(num_generations):
                self.logger.info(
                    f"Generation {len(results) + 1}/{total_prompts}: " f"Prompt {i+1}/{len(prompts)}, Run {gen_num+1}/{num_generations}"
                )
...
Supported model shortcuts:
  ...
  llama2-7b, llama2-13b, llama2-70b
    ...
    # Model configuration
    parser.add_argument("--model", "-m", default="llama-7b", help="Model name or shorthand (default: llama-7b)")

    parser.add_argument("--device", choices=["cuda", "cpu", "auto"], default="auto", help="Target device (default: auto)")

    parser.add_argument("--precision", choices=["float16", "float32", "int8"], default="float16", help="Model precision (default: float16)")

    parser.add_argument("--max-memory-mb", type=int, help="Maximum GPU memory to use in MB")

    # Generation parameters
    parser.add_argument("--prompt", "-p", help="Input text prompt")

    parser.add_argument("--max-tokens", type=int, default=50, help="Maximum new tokens to generate (default: 50)")

    parser.add_argument("--temperature", type=float, default=0.7, help="Sampling temperature (default: 0.7)")

    parser.add_argument("--top-p", type=float, default=0.9, help="Nucleus sampling parameter (default: 0.9)")
    ...
    parser.add_argument("--no-sampling", action="store_true", help="Use greedy decoding instead of sampling")

    # Benchmark options
    parser.add_argument("--benchmark", action="store_true", help="Run benchmark with default prompts")

    parser.add_argument("--num-generations", type=int, default=1, help="Number of generations per prompt (default: 1)")

    # Output options
    parser.add_argument("--output-file", "-o", help="Save results to JSON file")

    parser.add_argument("--log-level", choices=["DEBUG", "INFO", "WARNING", "ERROR"], default="INFO", help="Logging level (default: INFO)")

    parser.add_argument("--quiet", "-q", action="store_true", help="Suppress generated text output")
...
def main() -> None:
    ...
    try:
        ...
        if args.benchmark:
            ...
            results = engine.run_benchmark(DEFAULT_PROMPTS, num_generations=args.num_generations, **generation_kwargs)
        else:
            # Single prompt mode
        ...
        if not args.quiet:
            ...
            for i, result in enumerate(results):
                ...
                print(f"Tokens: {result['generated_tokens']} " f"({result['tokens_per_second']:.1f} tok/s)")
        ...
        if args.output_file:
            output_data = {"args": vars(args), "metrics": engine.get_metrics(), "results": results}
