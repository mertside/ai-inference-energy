...
class EnergyProfiler:
    ...
    def calculate_energy_from_power_time(self, 
                                       power: Union[float, np.ndarray, pd.Series],
                                       time: Union[float, np.ndarray, pd.Series]) -> Union[float, np.ndarray]:
        """
    ...
    def process_profiling_run(self, 
    ...
    def aggregate_multi_run_energy(self,
                                 run_data: List[pd.DataFrame],
                                 group_by_cols: List[str] = ['frequency'],
                                 energy_col: str = 'energy_joules') -> pd.DataFrame:
        """
        ...
        stats_df = combined_df.groupby(group_by_cols)[energy_col].agg([
    ...
    def load_and_process_profiling_files(self,
                                       file_paths: List[str],
                                       power_col: str = 'power',
                                       time_col: str = 'execution_time',
    ...
    def normalize_energy_measurements(self,
                                    energy_data: pd.DataFrame,
                                    energy_col: str = 'energy_joules_mean',
                                    baseline_frequency: Optional[int] = None,
                                    baseline_value: Optional[float] = None) -> pd.DataFrame:
        """
    ...
    def validate_energy_measurements(self,
                                   energy_data: pd.DataFrame,
                                   energy_col: str = 'energy_joules_mean',
    ...
    def calculate_fgcs_compatible_energy(self, 
                                       df: pd.DataFrame,
                                       power_col: str = 'power',
                                       time_col: str = 'execution_time',
                                       frequency_col: str = 'frequency') -> pd.DataFrame:
    ...
    def analyze_energy_efficiency_across_frequencies(self, 
                                                   df: pd.DataFrame,
                                                   power_col: str = 'power',
                                                   time_col: str = 'execution_time',
                                                   frequency_col: str = 'frequency') -> Dict[str, Any]:
        ...
        if frequency_col in df_energy.columns:
            ...
            for freq, group in freq_groups:
                freq_metrics = {
                    ...
                    'energy_efficiency': group['energy'].mean() / group[time_col].mean() if group[time_col].mean() > 0 else 0  # Energy per time
                }
            ...
            efficiency_analysis['optimization_insights'] = {
                ...
                'frequency_sweep_effectiveness': len(all_frequencies) / (max(all_frequencies) - min(all_frequencies)) * 100 if len(all_frequencies) > 1 else 0
            }
    ...
    def validate_energy_measurements(self, 
        ...
        if power_col in df.columns:
            ...
            power_outliers = df[(df[power_col] < power_q1 - 1.5 * power_iqr) | 
        ...
        if time_col in df.columns:
            ...
            time_outliers = df[(df[time_col] < time_q1 - 1.5 * time_iqr) | 
...
def validate_energy_data_quality(energy_data: pd.DataFrame,
                                energy_col: str = 'energy',
                                cv_threshold: float = 0.2) -> bool:
    """
