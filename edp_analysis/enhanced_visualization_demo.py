...
def create_feature_importance_data():
    ...
    try:
        ...
        features = [
            'fp_activity', 'dram_activity', 'sm_clock', 'frequency_squared',
            'fp_dram_interaction', 'log_frequency', 'power_density',
            'compute_intensity', 'memory_bandwidth_util', 'cache_hit_rate',
            'instruction_throughput', 'memory_latency', 'thermal_state',
            'voltage_level', 'workload_complexity'
        ]
...
def create_optimization_results(df):
    ...
    try:
        ...
        # Find optimal points
        avg_df = df.groupby('frequency').agg({
            'energy': 'mean',
            'execution_time': 'mean',
            'power': 'mean'
        }).reset_index()
