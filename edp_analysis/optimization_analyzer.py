...
class MultiObjectiveOptimizer:
    ...
    def __init__(self, energy_weight: float = 0.5, performance_weight: float = 0.5):
        ...
        logger.info(f"Multi-objective optimizer initialized: energy_weight={self.energy_weight:.3f}, "
    ...
    def find_pareto_frontier(self, 
                           ...
                           minimize_both: bool = True) -> pd.DataFrame:
        """
        ...
        for i, row in df_sorted.iterrows():
            ...
            for j, other_row in df_sorted.iterrows():
                ...
                if minimize_both:
                    ...
                    if (other_row[energy_col] <= row[energy_col] and other_row[time_col] <= row[time_col] and
                        (other_row[energy_col] < row[energy_col] or other_row[time_col] < row[time_col])):
                        is_dominated = True
                        break
                else:
    ...
    def weighted_sum_optimization(self,
                                ...
                                energy_col: str = 'energy',
                                time_col: str = 'execution_time',
                                normalize: bool = True) -> OptimizationResult:
        """
        ...
        if normalize:
            ...
            df_work['weighted_score'] = (self.energy_weight * df_work['energy_norm'] + 
                                       self.performance_weight * df_work['time_norm'])
        else:
            df_work['weighted_score'] = (self.energy_weight * df_work[energy_col] + 
    ...
    def constraint_based_optimization(self,
                                    df: pd.DataFrame,
                                    energy_col: str = 'energy',
                                    time_col: str = 'execution_time',
                                    max_energy: Optional[float] = None,
                                    max_time: Optional[float] = None,
                                    objective: str = 'energy') -> Optional[OptimizationResult]:
        """
...
class FGCSOptimizer:
    ...
    def edp_optimal(df: pd.DataFrame, 
        ...
        logger.info(f"EDP Optimal: f={result.frequency}MHz, t={result.execution_time}s, "
                   f"p={result.power}W, e={result.energy}J")
        
        return result
    
    @staticmethod
    def ed2p_optimal(df: pd.DataFrame,
                    energy_col: str = 'predicted_n_to_r_energy',
                    time_col: str = 'predicted_n_to_r_run_time',
                    energy_weight: float = 0.5, 
                    time_weight: float = 0.5) -> OptimizationResult:
        ...
        logger.info(f"ED²P Optimal: f={result.frequency}MHz, t={result.execution_time}s, "
                   f"p={result.power}W, e={result.energy}J")
...
class DVFSOptimizationPipeline:
    ...
    def _predict_power_across_frequencies(self, fp_activity: float, dram_activity: float, 
    ...
    def _predict_runtime(self, power_df: pd.DataFrame, baseline_runtime: float, 
                        fp_activity: float, frequencies: List[int]) -> pd.DataFrame:
