...
class EDPFeatureSelector:
    ...
    def select_features_for_edp(self, 
                               df: pd.DataFrame, 
    ...
    def _select_fgcs_features(self, df: pd.DataFrame) -> List[str]:
        ...
        fgcs_priority_features = [
            'fp_activity', 'dram_activity', 'n_sm_app_clock',
            'fp_x_clock', 'fp_activity_sq', 'n_sm_app_clock_sq'
        ]
...
def create_optimized_feature_set(df: pd.DataFrame, 
                                gpu_type: str = 'V100',
                                target_col: str = 'power',
                                max_features: int = 8) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
