...
def demo_enhanced_edp_calculation():
    ...
    try:
        from edp_analysis.edp_calculator import calculate_edp_with_features, analyze_feature_importance_for_edp
...
def demo_energy_profiling():
    ...
    try:
        ...
        logger.info(f"Added FGCS-compatible energy columns: {[col for col in df_energy.columns if col.startswith('n_') or col.startswith('predicted_')]}")
...
def demo_performance_profiling():
    ...
    try:
        ...
        fgcs_results = profiler.analyze_fgcs_performance_model(
            df, fp_activity=0.3, baseline_time=None, 
            time_col='execution_time', frequency_col='frequency'
        )
        ...
        if 'performance_insights' in fgcs_results:
            ...
            if insights:  # Check if insights dict is not empty
                ...
                logger.info(f"  Performance improvement potential: {insights.get('performance_improvement_potential', 0):.1f}%")
    ...
    try:
        ...
        if pareto_results:
            ...
            for i, result in enumerate(pareto_results[:3]):
                logger.info(f"  Pareto point {i+1}: {result.frequency} MHz, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu-type", default="V100", choices=["V100", "A100", "H100"],
