...
class GPUFeatureExtractor:
    ...
    def extract_basic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_columns = ['POWER', 'SMACT', 'SMOCC', 'FP32A', 'FP64A', 'DRAMA', 
                          'TEMP', 'MEMCLK', 'GRCLK', 'MEM_USED', 'MEM_FREE']
    ...
    def get_feature_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return summary_df.sort_values('importance_score', ascending=False) if 'importance_score' in summary_df.columns else summary_df


def process_dcgmi_data(df: pd.DataFrame, 
