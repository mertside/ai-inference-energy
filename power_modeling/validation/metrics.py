...
class CrossValidationAnalyzer:
    ...
    def validate_model(self, model, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        cv_scores = cross_val_score(
            model, X, y, cv=self.kfold, scoring='r2', n_jobs=-1
        )
...
class PowerModelValidator:
    ...
    def validate_power_model(self, model, X_test: np.ndarray, y_test: np.ndarray,
    ...
    def validate_frequency_predictions(self, model, fp_activity: float, 
                                     dram_activity: float, 
                                     frequencies: List[int]) -> Dict[str, Any]:
        """
