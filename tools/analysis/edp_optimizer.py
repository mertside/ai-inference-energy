...
GPU_MAX_FREQUENCIES = {"A100": 1410, "H100": 1785, "V100": 1380}  # MHz  # MHz  # MHz
...
class EDPOptimizer:
    ...
    def analyze_all_results(self) -> List[OptimalResult]:
        ...
        result_dirs = [d for d in self.results_dir.iterdir() if d.is_dir() and d.name.startswith("results_")]
    ...
    def print_summary(self, results: List[OptimalResult]):
        ...
        print(
            f"    • Configurations faster than max freq: {faster_than_max_edp}/{len(results)} ({100*faster_than_max_edp/len(results):.1f}%)"
        )
        ...
        print(
            f"    • Configurations faster than max freq: {faster_than_max_ed2p}/{len(results)} ({100*faster_than_max_ed2p/len(results):.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--results-dir", "-r", default="../../sample-collection-scripts", help="Path to sample-collection-scripts directory"
    )
    parser.add_argument(
