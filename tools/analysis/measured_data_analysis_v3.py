...
"""
Measured Data Analysis for AI Inference Energy Project - CORRECTED VERSION
...
- Keeps max frequency baseline logic for energy optimization
- Maintains statistical aggregation and outlier detection
"""
...
def extract_timing_from_out_file(file_path):
    """Extract timing information from .out file - CORRECTED to use inference timing."""
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            
            # Look for timing patterns - CORRECTED: Focus on inference timing, not total session time
            patterns = [
                r'Total Inference Time:\s*(\d+\.?\d*)\s*s',  # PRIMARY: Actual AI inference time
                r'Inference Time:\s*(\d+\.?\d*)\s*s',
                r'Model Inference Time:\s*(\d+\.?\d*)\s*s',
                r'Generation Time:\s*(\d+\.?\d*)\s*s',
                r'Forward Pass Time:\s*(\d+\.?\d*)\s*s',
                # Fallback patterns if above not found
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    print(f"    Found inference timing: {timing}s (pattern: {pattern.split(':')[0]})")
                    return timing
                    
        print(f"    No inference timing pattern found in {file_path}")
        return None
...
def process_result_directory(result_dir):
    ...
    for profile_file in profile_files:
        ...
        # CORRECTED: Find corresponding .out file for proper inference timing
        base_name = profile_file.stem.replace('_profile', '_app')
        out_file = profile_file.parent / f"{base_name}.out"
        
        if not out_file.exists():
            print(f"      Skipping - no .out file found: {out_file}")
            continue
        
        # CORRECTED: Extract inference timing from .out file
        execution_time = extract_timing_from_out_file(out_file)
        if not execution_time:
            print(f"      Skipping - no inference timing data")
            continue
...
def aggregate_measurements_by_frequency(measurements, exclude_first_run=True):
    ...
    for freq, runs in freq_groups.items():
        ...
        avg_measurement = {
            ...
            'data_source': 'measured_experimental_data_corrected_timing'
        }
...
def calculate_optimal_frequency(measurements, constraint_pct=5.0):
    ...
    # CORRECTED: Use maximum frequency as baseline (proper energy optimization reference)
    ...
    # CORRECTED: Proper performance impact interpretation with inference timing
...
def main():
    """Main analysis function with timing extraction and max frequency baseline"""
    print("=== MEASURED DATA ANALYSIS ===")
    print("Using proper inference timing from .out files with max frequency baseline")
    ...
    print("\n" + "="*80)
    print(" OPTIMAL FREQUENCY ANALYSIS - PROPER INFERENCE TIMING")
    print("="*80)
    ...
    for gpu in sorted(by_gpu.keys()):
        ...
        for workload in sorted(workloads.keys()):
            ...
            # CORRECTED: Proper performance impact display
    ...
    print(f"  • Timing source: Inference timing from .out files")
    ...
    print(f"\n✅ CORRECTED analysis complete with proper inference timing!")
    print("="*80)
