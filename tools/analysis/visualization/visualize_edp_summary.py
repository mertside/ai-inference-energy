...
class SummaryVisualizer:
    ...
    def __init__(self, results_file: str, output_dir: str = "edp-plots"):
    ...
    def create_energy_savings_comparison(self):
        ...
        ax.axhline(y=avg_edp, color=self.colors["edp"], linestyle="--", alpha=0.7, label=f"EDP Average: {avg_edp:.1f}%")
        ax.axhline(y=avg_ed2p, color=self.colors["ed2p"], linestyle="--", alpha=0.7, label=f"ED²P Average: {avg_ed2p:.1f}%")
    ...
    def create_frequency_optimization_comparison(self):
        ...
        # Create scatter plot
        scatter = ax.scatter(edp_freqs, ed2p_freqs, s=150, c=colors_list, alpha=0.7, edgecolors="black", linewidth=1.5)

        # Add diagonal reference line
        min_freq = min(min(edp_freqs), min(ed2p_freqs))
        max_freq = max(max(edp_freqs), max(ed2p_freqs))
        ax.plot([min_freq, max_freq], [min_freq, max_freq], "k--", alpha=0.5, linewidth=2, label="Equal Frequency")

        # Add annotations for each point
        for i, (edp_f, ed2p_f, label) in enumerate(zip(edp_freqs, ed2p_freqs, config_labels)):
            ax.annotate(label, (edp_f, ed2p_f), xytext=(5, 5), textcoords="offset points", fontsize=9, alpha=0.8)
        ...
        ax.set_title(
            "Frequency Selection Comparison: EDP vs ED²P Strategies\n" "Points below diagonal favor higher frequency for ED²P",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        for gpu, color in gpu_colors.items():
            ax.scatter([], [], c=color, s=150, alpha=0.7, edgecolors="black", linewidth=1.5, label=f"{gpu} GPU")
    ...
    def create_performance_impact_analysis(self):
        ...
        plt.suptitle(
            "Performance Impact Analysis of EDP and ED²P Optimization\n"
...
📊 Dataset: {len(self.results)} GPU-workload configurations
   ...
   Workloads: LLaMA, StableDiffusion, ViT, Whisper"""
        ...
        plt.suptitle(
            "EDP vs ED²P GPU Frequency Optimization: Comprehensive Analysis\n"
