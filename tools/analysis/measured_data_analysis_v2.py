...
def extract_timing_from_experiment_summary(result_dir, frequency, run_number):
    """Extract timing information from experiment_summary.log for a specific frequency and run."""
    experiment_summary_path = result_dir / "experiment_summary.log"
    
    if not experiment_summary_path.exists():
        print(f"    No experiment_summary.log found in {result_dir}")
        return None
    
    try:
        with open(experiment_summary_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        
        # Look for the Run Timing Summary section
        timing_section_match = re.search(r'Run Timing Summary:\s*=+\s*(.*?)(?=\n\n|\nFrequency|\nProfiling|\Z)', content, re.DOTALL)
        if not timing_section_match:
            print(f"    No Run Timing Summary section found")
            return None
        
        timing_section = timing_section_match.group(1)
        
        # Extract run timing for specific run number and frequency
        # Pattern: Run 1_01 : 383s (freq: 1410MHz, status: success)
        run_pattern = rf'Run\s+{run_number}_\d+\s*:\s*(\d+)s\s*\(freq:\s*{frequency}MHz,\s*status:\s*(\w+)\)'
        
        matches = re.findall(run_pattern, timing_section)
        if not matches:
            print(f"    No timing found for run {run_number} at {frequency}MHz")
            return None
        
        # Find the successful run
        for time_str, status in matches:
            if status.lower() == 'success':
                timing = float(time_str)
                print(f"    Found timing from experiment_summary.log: {timing}s (run {run_number}, {frequency}MHz)")
                return timing
        
        print(f"    No successful runs found for run {run_number} at {frequency}MHz")
        return None
        
    except Exception as e:
        print(f"Error reading timing from experiment_summary.log: {e}")
        return None
...
def process_result_directory(result_dir):
    ...
    for profile_file in profile_files:
        ...
        # Extract frequency and run info
        ...
        print(f"      Frequency: {frequency} MHz, Run: {run_number}")
        
        # Extract timing from experiment_summary.log
        execution_time = extract_timing_from_experiment_summary(result_dir, frequency, run_number)
        ...
        print(f"      ✓ Added: {frequency}MHz run {run_number}, {execution_time:.2f}s, {avg_power:.2f}W")
...
def aggregate_measurements_by_frequency(measurements, exclude_first_run=True):
    ...
    for freq in freq_groups:
        # Sort by filename or job_id to get consistent ordering
        freq_groups[freq] = sorted(freq_groups[freq], key=lambda x: (x['job_id'], x.get('run_number', 0)))
    ...
    for freq, runs in freq_groups.items():
        ...
        if exclude_first_run and len(runs) > 1:
            ...
            total_excluded += 1
            print(f"      Excluding first run at {freq}MHz (cold-start, time: {excluded_run['execution_time']:.2f}s)")
        ...
        avg_measurement = {
            ...
            'data_source': 'measured_experimental_data_outlier_filtered'
        }
...
def calculate_optimal_frequency(measurements, constraint_pct=5.0):
    ...
    # FIXED: Find baseline as MAXIMUM FREQUENCY (default GPU behavior)
    ...
    if not valid_frequencies:
        print(f"    No frequencies meet constraint - all frequencies slower than max frequency")
        return None
    ...
    # Enhanced performance impact interpretation
    if result['performance_degradation'] < -50:
        print(f"    🚀 MAJOR Performance improvement vs max frequency: {abs(result['performance_degradation']):.1f}% FASTER")
        print(f"    ⚠️  Maximum frequency appears to be throttling or suboptimal for this workload")
    elif result['performance_degradation'] < -10:
...
def main():
    """Main analysis function with statistical aggregation, cold-start exclusion, and outlier detection"""
    print("=== MEASURED DATA ANALYSIS WITH OUTLIER DETECTION ===")
    print("Using 100% measured experimental data with cold-start exclusion and outlier filtering\n")
    ...
    print("\n" + "="*80)
    print(" COMPREHENSIVE OPTIMAL FREQUENCY ANALYSIS - WARM RUNS ONLY")
    print("="*80)
    ...
    for gpu in sorted(by_gpu.keys()):
        ...
        for workload in sorted(workloads.keys()):
            ...
            # Enhanced performance impact display
    ...
    for workload in sorted(workload_comparison.keys()):
        ...
        if avg_perf_impact < 0:
            print(f"    • Average performance improvement: {abs(avg_perf_impact):.1f}% FASTER than max frequency")
        else:
    ...
    if min(all_perf_impacts) < 0:
        print(f"  • Performance vs max freq: {abs(max(all_perf_impacts)):.1f}% slower to {abs(min(all_perf_impacts)):.1f}% FASTER")
    else:
        print(f"  • Performance degradation range: {min(all_perf_impacts):.1f}% to {max(all_perf_impacts):.1f}%")
    ...
    if avg_perf < 0:
        print(f"  • Average performance improvement: {abs(avg_perf):.1f}% FASTER than max frequency")
        print(f"  • 🔥 KEY FINDING: Maximum GPU frequency is suboptimal for these AI workloads!")
    else:
        print(f"  • Average performance degradation: {avg_perf:.1f}%")
    
    print(f"\n✅ Analysis complete with outlier detection and statistical reliability!")
    print("="*80)
