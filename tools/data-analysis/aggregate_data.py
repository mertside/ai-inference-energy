...
class AIInferenceDataAggregator:
    ...
    def extract_features(self, profile_df: pd.DataFrame, timing_data: dict) -> dict:
        ...
        return features
        """Load DCGMI profile data from CSV file"""
        try:
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith('#Entity'):
                    header_line = line.strip()[1:].strip()  # Remove #
                    # Find first GPU data line
            ...
            if not header_line:
                logger.error(f"No header found in {profile_file}")
                return pd.DataFrame()
            ...
            for line in lines[data_start_idx:]:
                ...
                if line.startswith('GPU'):
                    ...
                    # Handle device name parsing (your visualization approach)
                    if len(parts) >= 7:
                        reconstructed = []
                        reconstructed.append(parts[0])  # GPU
                        reconstructed.append(parts[1])  # 0
                        
                        # Find power value to determine device name end
                        power_idx = None
            ...
            if not data_lines:
                logger.error(f"No data lines found in {profile_file}")
                return pd.DataFrame()
            ...
            # Convert numeric columns
            numeric_columns = list(self.dcgmi_fields.keys())
            for col in numeric_columns:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce')
                    
            logger.debug(f"Loaded {len(df)} samples from {profile_file.name}")
            return df
    ...
    def extract_features(self, profile_df: pd.DataFrame) -> Dict[str, float]:
        """Extract features from profile data using your proven approach"""
        if profile_df.empty:
            return {}
            
        features = {}
        
        # Your proven critical features (mean values across run)
        for feature_name, column_name in self.critical_features.items():
            dcgmi_column = None
            for dcgmi_col, mapped_name in self.dcgmi_fields.items():
                if mapped_name == column_name:
                    dcgmi_column = dcgmi_col
                    break
                    
            if dcgmi_column and dcgmi_column in profile_df.columns:
                # Use mean value (your approach for stable features)
                features[feature_name] = profile_df[dcgmi_column].mean()
            else:
                features[feature_name] = 0.0
                
        # Additional AI-specific features
        if 'TENSO' in profile_df.columns:  # Tensor core utilization
            features['tensor_core_utilization'] = profile_df['TENSO'].mean()
        if 'MCUTL' in profile_df.columns:  # Memory bandwidth utilization  
            features['memory_bandwidth_utilization'] = profile_df['MCUTL'].mean()
        if 'GPUTL' in profile_df.columns:  # GPU utilization
            features['gpu_utilization'] = profile_df['GPUTL'].mean()
            
        # Mixed precision ratio (AI-specific)
        fp16_active = profile_df['FP16A'].mean() if 'FP16A' in profile_df.columns else 0.0
        fp32_active = profile_df['FP32A'].mean() if 'FP32A' in profile_df.columns else 0.0
        total_fp = fp16_active + fp32_active
        features['mixed_precision_ratio'] = fp16_active / total_fp if total_fp > 0 else 0.0
