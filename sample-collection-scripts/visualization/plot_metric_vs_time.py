...
class ProfilingDataLoader:
    ...
    def load_csv_file(self, csv_path: Path, frequency: int) -> pd.DataFrame:
        ...
        try:
            ...
            df["timestamp"] = pd.date_range(start="2024-01-01", periods=len(df), freq="50ms")
...
class MetricPlotter:
    ...
    def plot_metric_vs_time(
        ...
        if metric not in df.columns:
            available_metrics = [
                col for col in df.columns if col not in ["Entity", "NVIDX", "DVNAM", "frequency", "normalized_time", "timestamp"]
            ]
        ...
        for freq, color in zip(frequencies, colors):
            ...
            if len(valid_data) > 0:
                ...
                if convert_to_percentage:
                    logger.debug(
                        f"{freq}MHz {metric}: {metric_values.min()*100:.1f}%-{metric_values.max()*100:.1f}% (mean: {metric_values.mean()*100:.1f}%)"
                    )
                else:
                    logger.debug(
                        f"{freq}MHz {metric}: {metric_values.min():.2f}-{metric_values.max():.2f} (mean: {metric_values.mean():.2f})"
                    )
...
Examples:
    ...
    parser.add_argument("--gpu", type=str, default="V100", help="GPU type (default: V100)")
    parser.add_argument("--app", type=str, default="LLAMA", help="Application name (default: LLAMA)")
    parser.add_argument(
        "--frequencies", type=str, default=None, help="Comma-separated frequencies in MHz (default: GPU-specific optimal set)"
    )
    parser.add_argument("--metric", type=str, default="GPUTL", help="Profiling metric to plot (default: GPUTL)")
    parser.add_argument("--run", type=int, default=1, help="Run number to read from (default: 1)")
    ...
    parser.add_argument("--save", type=str, default=None, help="Path to save the plot (default: auto-generate filename in plots/ folder)")
    parser.add_argument("--title", type=str, default=None, help="Custom plot title")
    parser.add_argument("--list-metrics", action="store_true", help="List available metrics for the specified configuration")
    parser.add_argument("--no-show", action="store_true", help="Don't show the plot (useful when saving)")
    ...
    if save_path is None:
        save_path = generate_plot_filename(
            gpu=args.gpu.upper(), app=args.app.upper(), metric=args.metric, frequencies=frequencies, run_number=args.run
        )
