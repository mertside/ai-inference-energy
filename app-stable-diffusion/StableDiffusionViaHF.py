...
import numpy as np
import torch
from diffusers import (  # === ADVANCED SCHEDULER COLLECTION ===
    DDIMScheduler,
    DDPMScheduler,
...
class StableDiffusionGenerator:
    ...
    BENCHMARK_PROMPTS = {
        "speed_test": ["A red car", "A blue house", "A green tree", "A cat", "A mountain"],
        "quality_test": [
    ...
    def _log_memory_usage(self, stage: str) -> None:
        ...
        if self.device == "cuda" and torch.cuda.is_available():
            ...
            self.logger.info(
                f"[{stage}] GPU Memory - Allocated: {memory_allocated:.2f}GB, "
                f"Reserved: {memory_reserved:.2f}GB, Total: {memory_total:.2f}GB"
            )

            self.generation_stats["memory_usage"].append(
                {"stage": stage, "allocated": memory_allocated, "reserved": memory_reserved, "total": memory_total}
            )
    ...
    def initialize_model(self) -> None:
        ...
        try:
            ...
            self.pipeline = self.pipeline_class.from_pretrained(self.model_id, **pipeline_kwargs)
    ...
    def _update_stats(self, generation_time: float, num_images: int) -> None:
        ...
        self.generation_stats["average_time"] = self.generation_stats["total_time"] / self.generation_stats["total_generations"]
    ...
    def run_comprehensive_benchmark(
        ...
        benchmark_stats = {
            ...
            "energy_research_metrics": {"baseline_measurements": [], "step_breakdown": [], "memory_efficiency": [], "batch_scaling": []},
        }
        ...
        if include_warmup:
            ...
            _ = self.generate_images(prompts=[prompts[0]], num_inference_steps=min(10, self.max_steps), batch_size=1)
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            for iteration in range(num_iterations):
                ...
                images = self.generate_images(prompts=[prompt], batch_size=1, seed=42 + iteration)  # Consistent but varied seeds
                ...
                benchmark_stats["performance_metrics"]["per_prompt_times"].append(
                    {"prompt_idx": prompt_idx, "iteration": iteration, "time": iteration_time, "images_generated": len(images)}
                )
...
🔬 EXAMPLE USAGE:
    ...
    model_group.add_argument("--custom-model", type=str, default=None, help="Custom Hugging Face model ID (overrides --model-variant)")
    ...
    scheduler_group.add_argument("--list-schedulers", action="store_true", help="List all available schedulers with descriptions")

    # === GENERATION PARAMETERS ===
    generation_group = parser.add_argument_group("🎨 Generation Parameters")
    generation_group.add_argument("--prompt", type=str, default=None, help="Text prompt for image generation")

    generation_group.add_argument("--negative-prompt", type=str, default=None, help="Negative prompt to avoid certain features")

    generation_group.add_argument("--height", type=int, default=None, help="Image height in pixels (default: model-specific)")

    generation_group.add_argument("--width", type=int, default=None, help="Image width in pixels (default: model-specific)")

    generation_group.add_argument("--steps", type=int, default=None, help="Number of inference steps (default: model-specific)")

    generation_group.add_argument(
        "--guidance-scale", type=float, default=7.5, help="Guidance scale for classifier-free guidance (1.0-20.0)"
    )

    generation_group.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility")

    # === BATCH PROCESSING ===
    batch_group = parser.add_argument_group("📦 Batch Processing")
    batch_group.add_argument("--num-images", type=int, default=1, help="Total number of images to generate")

    batch_group.add_argument("--batch-size", type=int, default=1, help="Batch size for generation (affects memory usage)")

    batch_group.add_argument("--images-per-prompt", type=int, default=1, help="Number of images to generate per prompt")

    # === BENCHMARK & RESEARCH ===
    research_group = parser.add_argument_group("🔬 Research & Benchmarking")
    research_group.add_argument("--benchmark", action="store_true", help="Run comprehensive benchmark suite")
    ...
    research_group.add_argument("--export-metrics", type=str, default=None, help="Export detailed performance metrics to JSON file")

    # === OPTIMIZATION SETTINGS ===
    optimization_group = parser.add_argument_group("⚙️ Optimization Settings")
    optimization_group.add_argument("--device", choices=["auto", "cuda", "cpu"], default="auto", help="Device for inference")

    optimization_group.add_argument(
        "--dtype", choices=["auto", "float16", "float32", "bfloat16"], default="auto", help="Model precision (affects memory and speed)"
    )

    optimization_group.add_argument(
        "--enable-cpu-offload", action="store_true", help="Enable CPU offloading for memory-constrained scenarios"
    )

    optimization_group.add_argument(
        "--disable-memory-efficient-attention", action="store_true", help="Disable memory efficient attention optimizations"
    )

    optimization_group.add_argument("--disable-xformers", action="store_true", help="Disable xformers memory efficient attention")

    optimization_group.add_argument("--disable-attention-slicing", action="store_true", help="Disable attention slicing optimization")

    # === OUTPUT CONFIGURATION ===
    output_group = parser.add_argument_group("💾 Output Configuration")
    output_group.add_argument("--output-dir", type=str, default=".", help="Output directory for generated images")

    output_group.add_argument("--output-prefix", type=str, default=None, help="Prefix for output filenames")

    output_group.add_argument("--job-id", type=str, default=None, help="Job ID for organizing images in subfolders (e.g., SLURM job ID)")

    output_group.add_argument("--save-metadata", action="store_true", help="Save generation metadata in image files")

    output_group.add_argument("--no-save-images", action="store_true", help="Skip saving images (research mode - metrics only)")

    # === ADVANCED RESEARCH ===
    advanced_group = parser.add_argument_group("🧪 Advanced Research Features")
    advanced_group.add_argument("--multi-resolution", action="store_true", help="Test multiple resolutions for memory scaling analysis")

    advanced_group.add_argument("--scheduler-comparison", action="store_true", help="Compare multiple schedulers for performance analysis")

    advanced_group.add_argument(
        "--energy-profile", action="store_true", help="Enable energy profiling integration (requires profiling framework)"
    )

    # === SYSTEM CONFIGURATION ===
    system_group = parser.add_argument_group("🔧 System Configuration")
    system_group.add_argument("--log-level", choices=["DEBUG", "INFO", "WARNING", "ERROR"], default="INFO", help="Logging verbosity level")

    system_group.add_argument("--quiet", action="store_true", help="Suppress non-essential output")

    system_group.add_argument(
        "--version", action="version", version="Stable Diffusion Research Tool v2.0 - Modernized for AI Energy Research"
    )
    ...
    parser.add_argument("--output-dir", type=str, default=".", help="Output directory for generated images")

    parser.add_argument("--batch-size", type=int, default=1, help="Batch size for generation")

    parser.add_argument("--benchmark", action="store_true", help="Run benchmark mode")

    parser.add_argument("--device", choices=["auto", "cuda", "cpu"], default="auto", help="Device to use for inference")

    parser.add_argument("--enable-cpu-offload", action="store_true", help="Enable CPU offloading for large models")
...
def main():
    ...
    try:
        ...
        if args.benchmark:
            ...
            images, stats = generator.run_benchmark_inference(num_generations=args.num_images, use_different_prompts=True)
        ...
        if not args.no_save_images:
            ...
            saved_files = generator.save_images(images, base_filename=output_filename, output_dir=output_dir)
